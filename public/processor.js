...
class MyProcessor extends SuperpoweredModule.AudioWorkletProcessor {
  ...
  onReady() {
    ...
    this.timeStretching = Superpowered.new(
      "TimeStretching",
  ...
  onMessageFromMainScope(message) {
    ...
    // changing the rate?
    if (typeof message.rate !== "undefined")
      this.timeStretching.rate = message.rate / 10000.0;
    // changing the pitch shift?
    if (typeof message.pitchShift !== "undefined")
      this.timeStretching.pitchShiftCents = message.pitchShift * 100;
  }
  ...
  processAudio(inputBuffer, outputBuffer, buffersize, parameters) {
    ...
    while (this.timeStretching.getOutputLengthFrames() < buffersize) {
      ...
      if (framesAvailable < 1) this.posFrames = 0;
